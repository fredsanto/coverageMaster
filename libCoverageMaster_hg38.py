...
reference_name = wd+"/REF/REFSEQ_hg38_c_HGMD3.gz"
...
def gene_reference(genes, reference, LOGFILE, nexons = 10):
    ...
    for g in genes:
        try:
            ...
            logreport("GENE NOT FOUND: '%s' is not present in the current reference database. Check the gene name for typos."%g,logfile = LOGFILE)
            #sys.exit(1)
    gs = []
    for g in gf:
        ...
        gdata = {'NM':g.split()[0],'chr':g.split('\t')[1],'expairs':expairs,'gene':g.split('\t')[-1],'start':exfirst,'end':exlast,'LONG':""}
